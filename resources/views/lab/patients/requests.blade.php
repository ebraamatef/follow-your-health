      ...
      <div class="col-md-12">
        <div class="pb-3 card border-0 rounded-0">
          ...
          <div class="card-header border-1 primary_color_border bg-white mb-2 p-0 fs-5">
              <a href="{{ URL('lab/requests/index') }}" class="me-1 float-end p-1 ps-3 pe-3  text-decoration-none text-white fs-6 primary_color">Requests</a>
              <a href="{{ URL('/lab/patients/find') }}" class="me-3 mb-0 float-end p-1 text-decoration-none text-dark fs-6 ">Find Patients</a>
              <a href="{{ URL('/lab/patients/index') }}" class="me-3 mb-0 float-end p-1 text-decoration-none text-dark fs-6 ">My Patients</a>
          </div>
            ...
            <div class="card-body">
                  ...
                  <div class="gap-3 row justify-content-center">
                      @if ($requests->isEmpty())
                        You have no requsts pending at the moment.
                        @else
                        @foreach($requests as $request)
                          <div class="p-0 card" style="width: 13rem;">
                              <div class="card-header">
                              <img src="@if($request->name == NULL) {{ URL('storage/patient.png') }}@else{{ URL("storage/patients/" . $request->user_id . "/" . $request->image) }}@endif" class="card-img-top" alt="...">
                              </div>
