      ...
      <div class="col-md-12">
        <div class="pb-3 card border-0 rounded-0">
          ...
          <div class="card-header border-1 primary_color_border bg-white mb-2 p-0 fs-5">
              ...
              <a href="{{ URL('/doctor/patients/index') }}" class="me-1 float-end p-1 ps-3 pe-3  text-decoration-none text-white fs-6 primary_color">My Patients</a>
          </div>
            ...
            <div class="card-body">
                  ...
                  <div class="gap-3 row justify-content-center">
                      @if ($myPatients->isEmpty())
                        <p class="text-center">You don't have any patients, find patients <a href="{{ URL('/doctor/patients/find') }}">here</a>.</p>
                        @else
                        @foreach($myPatients as $myPatient)
                          <div class="p-0 card" style="width: 13rem;">
                              <div class="card-header">
                              <img src="@if($myPatient->name == NULL) {{ URL('storage/patient.png') }}@else{{ URL("storage/patients/" . $myPatient->id. "/" . $myPatient->image) }}@endif" class="card-img-top" alt="...">
                              </div>
                              <div class="p-0 card-body text-center">
                                <p class="mb-0 mt-1"><b>{{ $myPatient->name  }}</b></p>
                                <a href="{{ URL('/doctor/patients/profile', $myPatient->id) }}" class="btn btn-primary primary_color border-0 mt-1 mb-2">
                                  View Profile
                                </a>
                              </div>
                          </div>
