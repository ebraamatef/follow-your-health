        ...
        <div class="pb-3 card border-0 rounded-3 shadow">
          <div class="card-header border-1 bg-white ps-3 pt-3 fs-4">
            ...
            <form class="mt-2 row g-3" action="{{ URL('/doctor/patient/search') }}" method="GET">
              @csrf
              <div class="col-auto">
                <a href="{{ URL('/doctor/home') }}" class="btn btn-primary mb-3" >Show My Patients</a>
              </div>
              <div class="col-6 col-sm-4 col-md-2">
                <select class="form-select" id="searchquery" name="search_in" placeholder="Search..." autocomplete="on">
                          ...
                          <div class="p-0 card" style="width: 13rem;">
                              ...
                              @for ($i = 0; $i < count($myPatients); $i++)
                                @if(in_array($patient->id, $request_ids))
                                  ...
                                  @break
                                  @elseif (dd(!in_array($patient->id, $myPatients) || empty($request_ids) || empty($myPatients) ))
                                  <form  action="{{ URL('/doctor/requests/create') }}" method="POST">
                                    @csrf
                                      <input type="hidden" value="{{ $patient->id }}" name="patient_id">
                                      <button type="submit" class="btn btn-primary">
