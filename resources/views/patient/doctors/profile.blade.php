                    ...
                    <div class="col-11 ">
                        <div class="row justify-content-center">
                            <div class="mb-4 col-8 col-md-3">
                                <img class="w-100" src="@if($doctor[0]->image == NULL) {{ URL('storage/doctor.png') }}@else{{ URL("storage/doctors/" . $doctor[0]->id . "/" . $doctor[0]->image) }}@endif">@if(in_array($doctor[0]->id, $myDoctors))
                                <div class="btn btn-danger float-start mt-2" data-bs-toggle="modal" data-bs-target="#deletemodal">Remove Doctor</div>
                                <a href="{{ URL('/patient/doctors/chat', $doctor[0]->id) }}" class="btn btn-primary primary_color primary_color_border float-end mt-2">Chat</a>
                                <form class="" action="{{url('/patient/doctors/remove')}}" method="POST">
                                    @csrf
                                    @method('DELETE')   
                                    <input type="hidden" name="patient_id"  value="{{ $doctor[0]->id }}" class="form-control" >
                                    <div class="modal" tabindex="-1" id="deletemodal">
                                        <div class="modal-dialog">
                                          <div class="modal-content">
                                            <div class="modal-header">
                                              <h5 class="modal-title"><b>Remove Doctor</b></h5>
                                              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                              <p>Are you sure you want to remove Dr. {{ $doctor[0]->name }} ?</p>
                                            </div>
