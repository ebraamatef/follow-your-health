...
<div class="mb-5" style="width: 100%; height: 150px; background-image:  url('{{ URL('/storage/doctor_banner.jpg') }}'); background-repeat: no-repeat; background-size: cover">
    <div class="d-flex align-items-center justify-content-center" style="width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5);">
        <div class='col-12 col-md-10 text-center'>
            <p class='text-white fs-1'><b>Find Doctors</b></p>
        </div>
    </div>
</div>
<div class="container">
      ...
      <div class="col-md-12">
        <div class="pb-3 card border-0 rounded-0">
          ...
          <div class="card-header border-1 primary_color_border bg-white mb-2 p-0 fs-5">
              ...
              <a href="{{ URL('/patient/doctors/find') }}" class="me-1 float-end p-1 ps-3 pe-3  text-decoration-none text-white fs-6 primary_color">Find Doctors</a>
              ...
              <div class="card-body">
                <div class="gap-3 row justify-content-center">
                    @if($doctors->isEmpty())
                        <p class="text-center fs-5">There are no results for "{{ $search_term }}".</p>
                      @else
                      @foreach ($doctors as $doctor)
                        <div class="p-0 card" style="width: 13rem;">
                            <div class="card-header">
                            <img style="aspect-ratio: 1/1" src="@if($doctor->image == NULL) {{ URL('storage/doctor.png') }}@else{{ URL("storage/doctors/" . $doctor->user_id . "/" . $doctor->image) }}@endif" class="card-img-top" alt="...">
                            </div>
                            <div class="p-0 card-body text-center">
                              <p class="mb-0 mt-1"><b>Dr. {{ $doctor->name }}</b></p>
                              <p class="mb-1">{{ $doctor->speciality }}</p>
                            <a href="{{ URL('/patient/doctors/profile', $doctor->id) }}" class="btn btn-primary primary_color border-0 mt-1 mb-2">
                              View Profile
                            </a>
                            </div>
                        </div>
