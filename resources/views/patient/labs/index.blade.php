...
<div class="mb-5" style="width: 100%; height: 150px; background-image:  url('{{ URL('/storage/doctor_banner.jpg') }}'); background-repeat: no-repeat; background-size: cover">
    <div class="d-flex align-items-center justify-content-center" style="width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5);">
        <div class='col-12 col-md-10 text-center'>
            <p class='text-white fs-1'><b>My Labs</b></p>
        </div>
    </div>
</div>
<div class="container">
      ...
      <div class="col-md-12">
        <div class="pb-3 card border-0 rounded-0">
          ...
          <div class="card-header border-1 primary_color_border bg-white mb-4 p-0 fs-5">
            ...
            <a href="{{ URL('/patient/labs/index') }}" class="me-1 float-end p-1 ps-3 pe-3  text-decoration-none text-white fs-6 primary_color">My Labs</a>
          </div>
            ...
            <div class="card-body">
                  ...
                  <div class="gap-3 row justify-content-center">
                      @if (empty($myLabs))
                        <p class="text-center">You don't have any labs, find labs <a href="{{ URL('/patient/labs/find') }}">here</a>.</p>
                        @else
                        @foreach($myLabs as $myLab)
                          <div class="p-0 card" style="width: 13rem;">
                              <div class="card-header">
                              <img src="@if($myLab->image == NULL) {{ URL('storage/lab.png') }}@else{{ URL("storage/labs/" . $myLab->user_id . "/" . $myLab->image) }}@endif" class="card-img-top" alt="...">
                              </div>
                              <div class="p-0 card-body text-center">
                              <p class="mb-0 mt-1"><b>{{ $myLab->name }}</b></p>
                              <p class="mb-1">{{ $myLab->type }}</p>
                              <a href="{{ URL('/patient/labs/profile', $myLab->id) }}" class="btn btn-primary primary_color border-0 mt-1 mb-2">
                                View Profile
                              </a>
                              </div>
                          </div>
            ...
            <div class="d-flex justify-content-center w-100 mb-0  mt-3">
              {{ $myLabs->appends(Request::all())->links() }}
          </div>
            </div>
        </div>
    </div>
